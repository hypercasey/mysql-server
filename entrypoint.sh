...
echo "[Entrypoint] MySQL Container Image 8.0.26-1"
...
if [ "$1" = 'mysqld' ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		# (`sudo podman run mysql/mysql-server:8.0 --default-time-zone=Europe/Berlin`)
		...
		if [ ! -z "$MYSQL_RANDOM_ROOT_PASSWORD" ]; then
			...
			MYSQL_ROOT_PASSWORD="$(cat /container-entrypoint-initdb.d/.my-password)"
			echo "[Entrypoint] GENERATED ROOT PASSWORD: $MYSQL_ROOT_PASSWORD"
			echo -e "\n[Entrypoint] GENERATED ROOT PASSWORD: $MYSQL_ROOT_PASSWORD\n" \
    			>>/var/log/mysql/mysqld.log
			echo -e "\n[Entrypoint] GENERATED ROOT PASSWORD: $MYSQL_ROOT_PASSWORD\n" \
    			>>/container-entrypoint-initdb.d/.my-password
		fi
...
EOF
		...
		if [ "$MYSQL_USER" ]; then
			echo "CREATE USER '"$MYSQL_USER"'@'%' REQUIRE X509;" | "${mysql[@]}"
			...
			echo "[Entrypoint] Not creating MySQL user."
		fi
		echo
